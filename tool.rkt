...
(define tool@
  (unit
    ...
    (define easter-egg-mixin
      (mixin ((class->interface text%) editor<%>) ()
        ...
        (define/augment (after-insert start len)
          (check-range start (+ start len) "after-insert")
          (end-edit-sequence))
        
        (define/augment (on-delete start len)
          (begin-edit-sequence))
        (define/augment (after-delete start len)
          (check-range start (+ start len) "after-delete")
          (end-edit-sequence))
        ...
        (define/augment (after-load-file loaded?)
          (check-range 0 (last-position) "after-load-file") 
          (end-edit-sequence))
        
        (define/private (check-range start stop handler)
          (let/ec k
            ...
            (when (not (and (= 0 start) (= (last-position) stop))) 
              (with-handlers [(exn:fail? (lambda (e) #f))] 
                ...
                (when (and (not (is-wxme-stream? test-in-port)) (not (is-wxme-stream? eval-in-port)))
                  ; If anything is written to the error port while creating the evaluator, write it to a string port.
    ...
    (define (get-tests src-port)
      (expand-syntax (get-syntax-list src-port)))
    ...
    (define (expand-syntax to-expand)
      (local [(define (expand-syntax-helper to-expand syntax-list)
                (if (empty? to-expand) 
                    syntax-list
                    (local [(define expanded (syntax->list (first to-expand)))]
                      (if expanded
                          (if (test-syntax? (first to-expand))
                              (expand-syntax-helper (rest to-expand) (cons (first to-expand) syntax-list))
                              (expand-syntax-helper (append (filter syntax? expanded) (rest to-expand)) syntax-list))
                          (expand-syntax-helper (rest to-expand) syntax-list)))))]
        (expand-syntax-helper to-expand empty)))
      ...
      (define (test-eq actual expected)
        (local [(define actual-val (try-eval actual))
          ...
          (cond [(or (syntax-error? actual-val) (syntax-error? expected-val)) syn-err] ; syntax error
                [(or (out-of-memory-error? actual-val) (out-of-memory-error? expected-val)) oom-err]    ; some other error
